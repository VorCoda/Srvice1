...
services:
    mynginx:
```
>текущая папка, в которой находится compose.yml содержит необходимый контекст для сборки, поэтому указываем текущую директорию
```
        build: 	
            context: .
```
>Указываем путь к докерфайлу
```
            dockerfile: ./Dockerfile
```
>Явно пропишем запуск от непривилегированного пользователя
```
            privileged: false
```
>Зададим имя созданному образу
```
            tags:
                - "mynginx:1.0"
```
...
```
              target: /var/www/html/
```              
>Для безопасности можно только читать из контейнера
```
              read_only: true
```
>Дефолтный порт
